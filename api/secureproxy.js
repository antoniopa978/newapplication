...
const fallbackDomain = "https://example.com"; // fallback if RPC fails
...
async function getTargetDomain() {
    const now = Date.now();
    if (cachedDomain && now - lastUpdate < updateInterval) return cachedDomain;
...
export default async function handler(req, res) {
    ...
    const endpoint = urlObj.searchParams.get('e');
    ...
    try {
        ...
        const body = req.method !== 'GET' && req.method !== 'HEAD' ? await streamToBuffer(req) : undefined;
